...
typedef struct
{
	...
	long lBuyDate;
	long lExpireDate;
} PMSG_REQ_PERIODITEMEX_INSERT, *LPPMSG_REQ_PERIODITEMEX_INSERT;
...
typedef struct
{
	...
	BYTE btItemType;
	char chCharacterName[MAX_IDSTRING+1];
} PMSG_REQ_PERIODITEMEX_DELETE, *LPPMSG_REQ_PERIODITEMEX_DELETE;
...
typedef struct
{
	...
	WORD wUserIndex;
	char chCharacterName[MAX_IDSTRING+1];
} PMSG_REQ_PERIODITEMEX_SELECT, *LPPMSG_REQ_PERIODITEMEX_SELECT;
...
} PMSG_REQ_LUCKYITEM_INSERT, *LPPMSG_REQ_LUCKYITEM_INSERT;
...
} PMSG_REQ_LUCKYITEM_INSERT_2ND, *LPPMSG_REQ_LUCKYITEM_INSERT_2ND;
...
typedef struct
{
	...
	WORD wItemCode;
	DWORD dwSerial;
} PMSG_REQ_LUCKYITEM_DELETE, *LPPMSG_REQ_LUCKYITEM_DELETE;
...
typedef struct
{
	...
	WORD wUserIndex;
	char chCharacterName[MAX_IDSTRING+1];
} PMSG_REQ_LUCKYITEM_SELECT, *LPPMSG_REQ_LUCKYITEM_SELECT;
...
class CPeriodBuffDBSet
{
public:
	CPeriodBuffDBSet();
	virtual ~CPeriodBuffDBSet();

	BOOL Connect();

	int Insert(char *szCharacterName, unsigned __int16 wBuffIndex, char btEffectType1, char btEffectType2, unsigned int dwDuration, int lExpireDate); // idb
	int Delete(char *szCharacterName, unsigned __int16 wBuffIndex); // idb
	int Select(char *szCharacterName, _tagPeriodBuffInfo *lpPeriodBuff, int *iBuffCnt); // idb
