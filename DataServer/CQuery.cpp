...
BOOL CQuery::Exec(LPCTSTR szSQL, BOOL bLog)
{
	int c;
...
BOOL CQuery::ReadInt64(LPCTSTR szSQL, INT64 &buf)
{
	SQLINTEGER LenBin;
	...
	{
		if( (ret=SQLGetData(hStmt,1,SQL_C_SBIGINT,&buf,sizeof(INT64), &LenBin))!=SQL_NO_DATA)
		{
			if (LenBin==SQL_NULL_DATA) 
			{
				Clear();
...
void CQuery::WriteInt64(LPCTSTR szSQL, INT64 *buf)
{
	...
	cbBlob=SQL_LEN_DATA_AT_EXEC(sizeof(INT64));

	SQLBindParameter(hStmt,1,SQL_PARAM_INPUT,SQL_C_SBIGINT,SQL_BIGINT,
		sizeof(INT64),0,(SQLPOINTER)1,0,&cbBlob);
	...
	if (ret==SQL_NEED_DATA) 
	{
		if ((int)pToken==1) 
		{
			SQLPutData(hStmt,(PTR)buf,sizeof(INT64));
		}
	}
